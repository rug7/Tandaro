...
<div className="relative">
  <TabsList className="flex w-full overflow-x-auto scrollbar-hide bg-gray-100 rounded-xl p-1 gap-1">
    <TabsTrigger value="table" className="rounded-lg font-semibold whitespace-nowrap flex-shrink-0 min-w-fit px-4">
      <Table className="w-4 h-4 mr-2 rtl:ml-2 rtl:mr-0" />
      {language === 'ar' ? 'الجدول' : language === 'he' ? 'טבלה' : 'Table'}
    </TabsTrigger>
    
    <TabsTrigger value="reservations" className="rounded-lg font-semibold whitespace-nowrap flex-shrink-0 min-w-fit px-4">
      <Calendar className="w-4 h-4 mr-2 rtl:ml-2 rtl:mr-0" />
      {language === 'ar' ? 'الحجوزات' : language === 'he' ? 'הזמנות' : 'Bookings'}
    </TabsTrigger>
    
    <TabsTrigger value="payments" className="rounded-lg font-semibold whitespace-nowrap flex-shrink-0 min-w-fit px-4">
      <CreditCard className="w-4 h-4 mr-2 rtl:ml-2 rtl:mr-0" />
      {language === 'ar' ? 'المدفوعات' : language === 'he' ? 'תשלומים' : 'Payments'}
    </TabsTrigger>
    
    <TabsTrigger value="vehicles" className="rounded-lg font-semibold whitespace-nowrap flex-shrink-0 min-w-fit px-4">
      <Truck className="w-4 h-4 mr-2 rtl:ml-2 rtl:mr-0" />
      {language === 'ar' ? 'المركبات' : language === 'he' ? 'רכבים' : 'Vehicles'}
    </TabsTrigger>
    
    <TabsTrigger value="gallery" className="rounded-lg font-semibold whitespace-nowrap flex-shrink-0 min-w-fit px-4">
      <BarChart3 className="w-4 h-4 mr-2 rtl:ml-2 rtl:mr-0" />
      {language === 'ar' ? 'المعرض' : language === 'he' ? 'גלריה' : 'Gallery'}
    </TabsTrigger>
    
    <TabsTrigger value="feedback" className="rounded-lg font-semibold whitespace-nowrap flex-shrink-0 min-w-fit px-4">
      <AlertCircle className="w-4 h-4 mr-2 rtl:ml-2 rtl:mr-0" />
      {language === 'ar' ? 'التقييمات' : language === 'he' ? 'משובים' : 'Feedback'}
    </TabsTrigger>
  </TabsList>
  
  {/* Scroll indicator for mobile */}
  <div className="absolute right-0 top-0 bottom-0 w-8 bg-gradient-to-l from-gray-100 to-transparent pointer-events-none rounded-r-xl lg:hidden"></div>
</div>
  ...
  <Gallery language={language} isAdmin={true} />
</TabsContent>
